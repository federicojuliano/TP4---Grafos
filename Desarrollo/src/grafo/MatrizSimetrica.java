...
public class MatrizSimetrica {
	...
	/**
	 * Carga una celda de la matriz de adyacencia. <br>
	 */
	public void setAdyacencia(int fila, int columna) {
		this.vector[cargarAdyacencia(fila,columna)] = true;
	}
	
	/**
	 * Devuelve la posicion de la celda a cargar. <br>
	 * 
	 * @return Posicion en Vector de Adyacencia. <br>
	 */
	public int cargarAdyacencia(int fila, int columna) {
		if(fila>columna)
			return columna*this.cantNodos+fila-(columna*columna+3*columna+2)/2;
		return fila*this.cantNodos+columna-(fila*fila+3*fila+2)/2;
	}
}
