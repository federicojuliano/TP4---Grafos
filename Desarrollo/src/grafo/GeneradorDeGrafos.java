...
//import grafos.Grafo;
//import grafos.PorcentajeException;
//import grafos.PosicionConRandom;
...
public class GeneradorDeGrafos {
	...
	public static void generarGrafoAleatorioPorcentajeAdyacencia(final String path, final int cantidadNodos,
		...
		grafo.cantidadAristas = (int) ((procentajeAdyacencia * cantMaxAristas) / 100);
		
		Random rnd = new Random();
		CeldaRandom [] posicionRandom = new CeldaRandom[ grafo.cantidadNodos*(grafo.cantidadNodos-1)/2 ];
		int k = 0;
		for(int i = 0; i < grafo.cantidadNodos-1; i++){
			for(int j = i+1; j < grafo.cantidadNodos; j++){
				posicionRandom[k] = new CeldaRandom(i,j,rnd.nextDouble());
				k++;
			}
		}
		
		Arrays.sort(posicionRandom);
		
		for(int i=0; i < grafo.cantidadAristas; i++) 
			grafo.setArista(posicionRandom[i].getFila(), posicionRandom[i].getColumna());
		
		grafo.porcentajeAdyacencia = grafo.obtenerAdyacenciaReal();
		grafo.gradoMax = 0;
